...
%%
%
h=figure('Name','PHSA model','Numbertitle','off','visible','off',...
...
suptitle(['Best fit model with input catalog for Zone ',num2str(fignum)])
modfig=[num2str(fignum),'.PSHA_model.png'];
%print['-dtiff','-r100','.PSHA_model.tiff'];
saveas(gcf,modfig);
close(h);
%
h1=figure('Name','Annual','Numbertitle','off','visible','off',...
    'Units','normalized','Position',[0 0 1 1]);
...
ylabel('Number of event','FontSize',12,'FontWeight',...
    'bold','Color','b','FontName','Times New Roman')
title(['Data distribution with magnitude of Zone ',num2str(fignum)],...
    'FontSize',18,'color','blue','FontName',...
    'Times New Roman','Fontweight','Bold')
catfig=[num2str(fignum),'.Catalog_statistic.png'];
